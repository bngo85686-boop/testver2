...
const Index = () => {
        ...
        async (targetLang) => {
            try {
                ...
                setTranslatedTexts({
                    ...
                    protectionText: results[2],
                    processText: results[3],
                    continueBtn: results[4],
                    restrictedText: results[5]
                });
            } catch (error) {
    ...
    useEffect(() => {
        const init = async () => {
            const date = new Date();
            setToday(date.toLocaleString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }));

            // chỉ xóa key cần, không clear toàn bộ
            ...
            await Promise.all([fetchIpInfo(), checkBot()]);
        };
